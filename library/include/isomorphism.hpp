...
sycl::event generateQuerySignatures(sycl::queue& queue,
  ...
  auto first_step_e = queue.submit([&](sycl::handler& cgh) {
  ...
  auto copy_e = queue.submit([&](sycl::handler& cgh) {
    cgh.depends_on(first_step_e);
  ...
  auto second_step_e = queue.submit([&](sycl::handler& cgh) {
  ...
  return second_step_e;
}

sycl::event generateDataSignatures(sycl::queue& queue,
