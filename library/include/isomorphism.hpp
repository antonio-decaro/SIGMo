...
protected:
  ...
  sycl::event pruneVertices(size_t max_local_size) {
    ...
    auto e = queue.submit([&](sycl::handler& cgh) {
      ...
      cgh.parallel_for<class PruneVertices>(sycl::nd_range<1>{global, local}, 
                                            [
                                              =,
                                              n_query = this->sycl_query_graphs.getNumGraphs(),
                                              mask_size=MASK_SIZE, 
                                              arrmask_lenght=this->candidates->getSingleMaskLenght(), 
                                              data_size=sycl_data_graph.getNumNodes()
                                            ] (sycl::nd_item<1> id) [[intel::reqd_sub_group_size(sg_size)]] 
      {
        ...
        for (int query_graph = sg_id; query_graph < n_query; query_graph += n_sg) {
          ...
          for (int data_node = lid; data_node < data_size; data_node += sg_size) {
            ...
            for (auto query_node = 0; query_node < query_graph_size; query_node++) {
              auto mask = msm::device::bitmask::getAtomicRef(candidates, offset + query_node, data_node, arrmask_lenght, mask_size);
              auto query_node_label = query_labels[offset + query_node];
              if (msm::device::graph::labelMatch(query_node_label, data_node_label)) {
                ...
                if (query_node_end - query_node_begin > data_node_end - data_node_begin) {
                  msm::device::bitmask::setOff(mask, data_node, mask_size);
                }
              } else {
                msm::device::bitmask::setOff(mask, data_node, mask_size);
              }
            ...
            for (auto query_node = 0; query_node < query_graph_size; query_node++) {
              auto mask = msm::device::bitmask::getAtomicRef(candidates, offset + query_node, data_node, arrmask_lenght, mask_size);
              if (msm::device::bitmask::get(candidates, offset + query_node, data_node, arrmask_lenght, mask_size)) {
                if (!msm::device::graph::isIsomorphic(query_node, data_node, offset, data_row_offsets, query_row_offsets, data_col_indices, query_col_indices, data_labels, query_labels, candidates, arrmask_lenght, mask_size)) {
                  msm::device::bitmask::setOff(mask, data_node, mask_size);
                }
              }
            }
  ...
  std::vector<std::pair<node_t, node_t>> findSolutions(size_t graph_id) {
    ...
    auto backtrack = [=](auto&& self,
                         node_t query_node, 
                         node_t data_node, 
                         std::set<node_t> query_visited, 
                         std::set<node_t>& data_visited) -> std::vector<std::pair<node_t, node_t>> {
      ...
      for (auto& query_neighbour : query_graph.getNeighbours(query_node)) {
        if (query_visited.find(query_neighbour) == query_visited.end()) {
          unvisited_neighbours.push_back(query_neighbour);
        }
      }
      if (unvisited_neighbours.empty()) {
        return {{query_node, data_node}};
      }
      ...
      for (auto& query_neighbour : unvisited_neighbours) {
        ...
        for (auto& data_neighbour : potential_mapping) {
          if (data_visited.find(data_neighbour) != data_visited.end()) {
            continue;
          }
        ...
        if (neighbour_res.empty()) {
          return {};
        }
        res.insert(res.end(), neighbour_res.begin(), neighbour_res.end());
        for (auto& [query_node, data_node] : neighbour_res) {
          data_visited.insert(data_node);
        }
      }
      if (res.empty()) {
  ...
  sycl::event pruneEdges(size_t max_local_size) {
    ...
    return queue.submit([&](sycl::handler& cgh) {
      ...
      cgh.parallel_for<class PruneEdges>(sycl::nd_range<1>{global, local}, 
                                        [=,
                                        n_query = this->sycl_query_graphs.getNumGraphs(),
                                        mask_size=MASK_SIZE, 
                                        arrmask_lenght=this->candidates->getSingleMaskLenght(), 
                  ...
                  if (frontier & neighbour_bit) {
                    ...
                    for (int c2 = candidates_offset[offset + query_neighbour]; c2 < candidates_offset[offset + query_neighbour + 1]; c2++) {
                      ...
                      for (int c1 = candidates_offset[offset + query_node]; c1 < candidates_offset[offset + query_node + 1]; c1++) {
                        ...
                        if (msm::device::graph::isNeighbour(data1, data2, data_row_offsets, data_col_indices)) {
                          found = true;
                          break;
                        }
                      }
                      if (!found) {
                        msm::device::bitmask::setOff(candidates, offset + query_neighbour, data2, arrmask_lenght, mask_size);
                      }
                    }
...
public:
  SubgraphIsomorphism(Graph& data_graph, std::vector<Graph>& query_graphs, sycl::queue& queue) :
                      data_graph(data_graph), compressed_query_graphs(query_graphs), queue(queue),
                      sycl_query_graphs(compressed_query_graphs), sycl_data_graph(data_graph) {
    size_t offset = 0;
  ...
  //TODO
  const std::vector<std::vector<std::pair<node_t, node_t>>> getAnswers() const {
    return answare;
  }
   ...
   */
  inline const double getExceutionTime() const {
    return time.count();
  }
};
