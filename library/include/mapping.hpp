...
public:
  BitmaskMap(size_t source_size, size_t target_size, bool init = false) noexcept :
    source_size(source_size), target_size(target_size), single_node_mask_lenght(target_size / MASK_SIZE + 1), masks_buffer(sycl::range{source_size * single_node_mask_lenght}) {
    ...
    for (int i = 0; i < single_node_mask_lenght; i++) {
      if (masks[offset + i] != 0) {
        for (int j = 0; j < MASK_SIZE && (i * MASK_SIZE + j) < target_size; j++) {
          if (get(source_node, i * MASK_SIZE + j, masks)) {
            on_nodes.push_back(i * MASK_SIZE + j);
          }
        }
      }
    }
  ...
  sycl::host_accessor<mask_t, 1> getHostAccessor() {
    return sycl::host_accessor {masks_buffer};
  }

  size_t size() {
    return source_size * single_node_mask_lenght;
  }
