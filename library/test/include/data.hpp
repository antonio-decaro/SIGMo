...
mbsm::signature::Signature<> expected_query_signatures[]{// Graph 1
...
mbsm::signature::Signature<> expected_data_signatures[]{// Graph 1
...
std::vector<mbsm::signature::Signature<>> getExpectedDataSignatures(std::string fname, size_t refinement_steps = 1) {
  auto data = mbsm::io::loadDataGraphsFromFile(fname);

  size_t num_nodes = 0;
  for (auto& graph : data) { num_nodes += graph.getNumNodes(); }

  std::vector<mbsm::signature::Signature<>> signatures(num_nodes);
  ...
  for (int _ = 0; _ < refinement_steps; _++) {
    std::vector<mbsm::signature::Signature<>> db_signatures(num_nodes);
    ...
    for (auto& graph : data) {
      for (int i = 0; i < graph.getNumNodes(); ++i) {
        ...
        for (int j = start_node; j < end_node; ++j) {
          auto neighbor = graph.getColumnIndices()[j];
          for (int l = 0; l < mbsm::signature::Signature<>::getMaxLabels(); ++l) {
...
std::vector<mbsm::signature::Signature<>> getExpectedQuerySignatures(std::string fname, size_t refinement_steps = 1) {
