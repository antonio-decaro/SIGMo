...
sigmo::signature::Signature<>::SignatureDevice expected_query_signatures[]{// Graph 1
...
sigmo::signature::Signature<>::SignatureDevice expected_data_signatures[]{// Graph 1
...
std::vector<sigmo::signature::Signature<>::SignatureDevice> getExpectedDataSignatures(std::string fname, size_t refinement_steps = 1) {
  auto data = sigmo::io::loadCSRGraphsFromFile(fname);

  size_t num_nodes = 0;
  for (auto& graph : data) { num_nodes += graph.getNumNodes(); }

  std::vector<sigmo::signature::Signature<>::SignatureDevice> signatures(num_nodes);
  ...
  for (int _ = 0; _ < refinement_steps; _++) {
    std::vector<sigmo::signature::Signature<>::SignatureDevice> db_signatures(num_nodes);
...
std::vector<sigmo::signature::Signature<>::SignatureDevice> getExpectedQuerySignatures(std::string fname, size_t refinement_steps = 1) {
  auto data = sigmo::io::loadAMGraphsFromFile(fname);
  ...
  for (auto& graph : data) {
    for (int i = 0; i < graph.getNumNodes(); ++i) {
      sigmo::types::node_t neighbors[sigmo::types::MAX_NEIGHBORS];
